    ...
    <table class="table table-bordered table-striped table-hover" id="tblmaster">
      ...
      <tbody>
        @foreach($datas as $data)
              <td class=" ">0{{$data->kd_skel}}</td>
              <td class=" ">{{$data->ur_skel}}</td>
              <td class=" last">

                  <a href="{{url('dataMaster/'.$data->kd_gol.'/'.$data->kd_bid.'/'.$data->kd_kel.'/'.$data->kd_skel)}}" class="btn btn-success btn-xs">Lihat</a>
              </td>
              </tr>
        @endforeach
      </tbody>
