...
<div class="x_panel">
  <div class="x_title">
    <h2>Daftar Kode Barang Aset -- {{$nama}}</h2>
    <ul class="nav navbar-right panel_toolbox">
      @if (Sentinel::getUser()->hasAccess(['datamaster.create']))
        <a href="{{route('datamaster.create')}}" class="btn btn-success">Tambah Barang</a>
      @endif
    </ul>
    ...
    <table class="table table-bordered table-striped table-hover" id="tblmaster">
      ...
      <tbody>
        @foreach($datas as $data)
            <td class=" ">0{{$data->kd_brg}}</td>
              <td class=" ">{{$data->ur_sskel}}</td>
              <td class=" last">

                  {{--<a href="{{url('dataMaster/'.$data->kd_gol.'/'.$data->kd_bid.'/'.$data->kd_kel)}}" class="btn btn-success btn-xs">Lihat</a>--}}
              </td>
              </tr>
        @endforeach
      </tbody>
