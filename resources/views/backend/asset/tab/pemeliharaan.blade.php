                ...
                <div class="message_wrapper">
                    ...
                    <p class="url">
                        @if (Sentinel::getUser()->hasAccess(['pemeliharaan.destroy']))
                          ...
                          <a onclick="hapuspelihara({{$p->id}})" href="javascript:void(0);" class="btn btn-danger"> <i class="fa fa-trash"></i> Delete</a>
                        @endif
                        @if (Sentinel::getUser()->hasAccess(['pemeliharaan.edit']))
                          <span class="fs1 text-info" aria-hidden="true" data-icon=""></span>
                          <a href="{{route('pemeliharaan.edit', $p->id)}}" class="btn btn-success"> <i class="fa fa-pencil"></i> Edit</a>
                        @endif
                        @if($p->dokumen_path != null)
                          <span class="fs1 text-info" aria-hidden="true" data-icon=""></span>
                          <a href="#" class="btn btn-info"><i class="fa fa-paperclip"></i> Unduh Dokumen Pendukung </a>
                        @else
                          <a href="javascript:void(0);" class="btn btn-danger"><i class="fa fa-paperclip"></i> Unggah Dokumen</a>
...
<script type="text/javascript">
  function hapuspelihara(id) {
    var link = "<?php echo Request::root(); ?>";
    document.getElementById('fhapus').action = link+'/pemeliharaan/'+id;
    if(confirm("Yakin Ingin Menghapus Pemeliharaan Ini?")==true){
